...
CREATE INDEX IF NOT EXISTS photo_backup_jobs_last_error ON photo_backup_jobs(account_id,drive_id,updated_at DESC) WHERE last_error<>''`)
	...
	_, err = e.db.ExecContext(ctx, "PRAGMA user_version=2")
	return err
}
...
func (e *Engine) PutSettings(ctx context.Context, s Settings) error {
	...
	_, err := e.db.ExecContext(ctx, `INSERT INTO photo_backup_settings(account_id,drive_id,enabled,photos,videos,future_only,wifi_only,charging_only,destination_parent_id,encrypt,manual_paused) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(account_id,drive_id) DO UPDATE SET enabled=excluded.enabled,photos=excluded.photos,videos=excluded.videos,future_only=excluded.future_only,wifi_only=excluded.wifi_only,charging_only=excluded.charging_only,destination_parent_id=excluded.destination_parent_id,encrypt=excluded.encrypt`, s.Scope.AccountID, s.Scope.DriveID, s.Enabled, s.Photos, s.Videos, s.FutureOnly, s.WiFiOnly, s.ChargingOnly, s.DestinationParentID, s.Encrypt, s.ManualPaused)
	return err
}
func (e *Engine) GetSettings(ctx context.Context, scope Scope) (Settings, error) {
	...
	err := e.db.QueryRowContext(ctx, `SELECT enabled,photos,videos,future_only,wifi_only,charging_only,destination_parent_id,encrypt,manual_paused FROM photo_backup_settings WHERE account_id=? AND drive_id=?`, scope.AccountID, scope.DriveID).Scan(&s.Enabled, &s.Photos, &s.Videos, &s.FutureOnly, &s.WiFiOnly, &s.ChargingOnly, &s.DestinationParentID, &s.Encrypt, &s.ManualPaused)
	return s, err
}
