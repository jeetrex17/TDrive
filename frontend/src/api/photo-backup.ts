...
export interface PhotoBackupStatus { phase: 'idle' | 'scanning' | 'queued' | 'uploading' | 'paused' | 'complete' | 'failed'; pending: number; uploading: number; complete: number; failed: number; paused: number; bytesDone: number; bytesTotal: number; message: string; }
...
export interface PhotoBackupState { settings: PhotoBackupSettings; sources: PhotoBackupSource[]; status: PhotoBackupStatus; capabilities: PhotoBackupCapabilities; platform: string; destination: { id: string; title: string; kind: string }; manualPaused: boolean; }
