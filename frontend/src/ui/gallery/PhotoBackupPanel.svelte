...
<style>
    ...
    .backup-progress { display: grid; gap: 5px; } .backup-progress strong { color: var(--text-main); font-size: 12px; } .backup-progress span { color: var(--text-muted); font-size: 12px; } .backup-current-file { color: var(--text-main); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } progress { width: 100%; accent-color: var(--accent); } .backup-actions { justify-content: flex-start; flex-wrap: wrap; } .backup-actions button { width: auto; min-height: 34px; padding: 7px 10px; display: inline-flex; align-items: center; gap: 6px; font-size: 12px; } .backup-actions button:disabled { cursor: not-allowed; opacity: .48; }
