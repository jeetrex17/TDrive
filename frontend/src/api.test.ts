...
describe("api normalizers", () => {
    ...
    it("toFolderItem maps fields", () => {
        const d = { id: "d1", name: "Docs", parent_id: "root" };
    ...
    it("toSearchHit clamps type to file|folder", () => {
        const searchHit = (partial: Partial<Parameters<typeof toSearchHit>[0]>) =>
            partial as unknown as Parameters<typeof toSearchHit>[0];
        expect(toSearchHit(searchHit({ type: "folder", id: "d" })).type).toBe("folder");
        expect(toSearchHit(searchHit({ type: "file", id: "5" })).type).toBe("file");
        expect(toSearchHit(searchHit({ type: "weird", id: "5" })).type).toBe("file");
    });
